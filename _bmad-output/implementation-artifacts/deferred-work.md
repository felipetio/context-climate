...
## Deferred from: code review of 5-4-search-local-indicators-mcp-tool (2026-05-30)

- **Relevance-quality gaps in the scoring algorithm** (`mcp_server/indicator_cache.py:67-109`) — the cascade is implemented verbatim per spec, but has known quality edges deferred to Story 5.5's test/audit pass: (a) no minimum-query-length guard, so a 1-2 char query substring-matches a large fraction of codes all at score 90; (b) no query-length cap, so a pathologically long query causes an O(query_words × records × name_words) CPU spike; (c) uses `str.lower()` rather than `str.casefold()` and applies no Unicode normalization, so some non-ASCII case-insensitive matches are missed. All are refinements to a spec-mandated algorithm, not defects against the current ACs.
- **Lazy-singleton race + blocking I/O + no defensive copy** (`mcp_server/indicator_cache.py:40-49`) — pre-existing items already tracked for `indicator_cache.py` (the spec's References point here): the `if _metadata_indicators is None` check-then-load has no lock (concurrent cold first-calls can read+parse the file twice; idempotent, not corrupting); `_load_json` is sync blocking I/O inside an async tool; and `get_metadata_indicators()` returns the live cached list by reference, so a caller could mutate the process-wide singleton. The tool path itself (`search_local_metadata`) builds fresh result dicts and does not leak references. Do not silently fix in Story 5.4 per the spec's deferral note.
