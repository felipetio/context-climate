...
- [x] [Review][Defer] **`open_dossier_canvas()` is not idempotent** [app/chat.py:58-67] — Calling it twice (e.g. via future resume path) creates a second `CustomElement` and overwrites `user_session["doc"]`, orphaning the first. _Severity: Low, deferred — will be addressed by **Story 10.5** (dossier canvas reopen affordance), which needs an idempotent reopen helper anyway._ (Tracked: see `_bmad-output/planning-artifacts/epics.md#story-105-dossier-canvas-reopen-affordance` — Story 10.5 AC1 fixes this.)
